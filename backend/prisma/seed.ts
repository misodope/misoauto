import { PrismaClient, PlatformType, VideoStatus, PostStatus } from '@prisma/client';
...
async function main() {
  ...
  for (const user of users) {
    const userPlatforms = faker.helpers.arrayElements(platforms, { min: 2, max: 4 });
    ...
    for (let i = 0; i < videoCount; i++) {
      const video = await prisma.video.create({
        data: {
          ...
          mimeType: 'video/mp4',
          status: VideoStatus.READY, // Using direct enum value instead of random
          userId: user.id,
        },
      });
        ...
        if (faker.number.int({ min: 0, max: 1 })) {
          await prisma.videoPost.create({
            data: {
              ...
              status: PostStatus.PUBLISHED, // Using direct enum value instead of random
